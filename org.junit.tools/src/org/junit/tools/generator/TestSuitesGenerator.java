...
public class TestSuitesGenerator implements ITestSuitesGenerator {
	...
	public boolean generateTestSuites(JUTElements jutElements) throws CoreException, JUTWarning {
		...
		testSuiteExists = generateTestSuites(testProject, testPackage, testClassName, childTestSuites);
	...
	private class TestSuiteDeclaration {
		...
		public HashSet<String> createChildTestSuiteNameList() {
			...
			for (TestSuiteDeclaration tsd : childTestSuiteDeclarations) {
				childTestSuiteNameList.add(tsd.getPackageName() + "." + tsd.getName() + ".class");
			}
	...
	private HashSet<String> createRootTestSuiteNameList(List<TestSuiteDeclaration> tsdList) {
		...
		for (TestSuiteDeclaration tsd : tsdList) {
			rootTestSuiteNameList.add(tsd.getPackageName() + "." + tsd.getName() + ".class");
		}
	...
	public boolean generateTestSuites(final IJavaProject testProject) throws OperationCanceledException, CoreException {
		...
		JavaCore.run(new IWorkspaceRunnable() {
			...
			private void generateTestSuites(IProgressMonitor monitor, IPackageFragmentRoot testFragmentRoot)
					throws JavaModelException, OperationCanceledException, CoreException {
				...
				for (IJavaElement javaElement : testFragmentRoot.getChildren()) {
					...
					if (javaElement instanceof IPackageFragment) {
						...
						if (!pf.getElementName().endsWith(".mock")) {
							...
							if (pf.getElementName().equals(pf.getJavaProject().getElementName() + ".all")) {
								...
								continue;
							} else if (pf.getElementName().equals("all")) {
								boolean baseTestSuiteFound = false;
								for (ICompilationUnit cu : pf.getCompilationUnits()) {
									if (cu.getElementName().startsWith("TestSuiteAll")) {
										baseTestSuitePackage = pf;
										baseTestSuiteCuList = pf.getCompilationUnits();
										baseTestSuiteFound = true;
										break;
									}
								}

								if (baseTestSuiteFound) {
									continue;
								}
							}
							...
							} else if (tsdBefore.getParent() != null && JDTUtils
									.isParentPackage(tsdBefore.getParent().getPackageName(), pf.getElementName())) {
					...
					if (baseTestSuitePackage == null) {
						try {
							baseTestSuitePackage = JDTUtils.getPackage(testProject, testFragmentRoot,
									testProject.getElementName() + ".all", true);
						} catch (JavaModelException ex) {
							// the package-name is not allowed, so we take only
							// the package-name 'all'
							baseTestSuitePackage = JDTUtils.getPackage(testProject, testFragmentRoot, "all", true);
						}
						baseTestSuiteCuList = new ICompilationUnit[0];
					}

					// create base-test-suite
					processTestSuiteDeclaration("TestSuiteAll", baseTestSuitePackage, baseTestSuiteCuList,
	...
	private void processTestSuiteDeclaration(TestSuiteDeclaration tsd)
		...
		processTestSuiteDeclaration(tsd.getName(), testSuitePackage, tsd.getCuList(),
				tsd.createChildTestSuiteNameList());
	...
	private void processTestSuiteDeclaration(String testSuiteName, IPackageFragment testSuitePackage,
			ICompilationUnit[] cuList, HashSet<String> childTestSuiteNameList)
			throws OperationCanceledException, CoreException {
		ICompilationUnit testSuite = testSuitePackage.getCompilationUnit(testSuiteName + ".java");

		// create new source
		IType testSuiteFrame = createTestSuiteFrame(testSuiteName, testSuite, cuList, null, childTestSuiteNameList,
				true);
	 ...
	 * @throws CoreException
	 */
	private boolean generateTestSuites(IJavaProject testProject, IPackageFragment testPackage,
			String newTestSuiteElementName, HashSet<String> childTestSuites) throws CoreException {
		...
		// create new source
		IType testSuiteFrame = createTestSuiteFrame(testSuiteName, testSuite, testClasses, newTestSuiteElementName,
				childTestSuites, false);
		if (testSuiteFrame == null) {
			return true;
		}
	 ...
	 */
	protected String createCommaSeparatedClassList(ICompilationUnit[] testClasses,
			HashSet<String> generatedTestSuites) {
	...
	protected String createCommaSeparatedClassList(ICompilationUnit[] testClasses, HashSet<String> generatedTestSuites,
		...
		for (ICompilationUnit testClass : testClasses) {
			...
			testClassNameWithClassSuffix = testClass.getElementName().replace(".java", ".class");
			testClassName = testClassNameWithClassSuffix.replace(".class", "");

			if (testClassName.equals(TESTSUITE_PREFIX) || !isValidTestClassName(testClassName)) {
				continue;
			}
	...
	@Override
	public boolean deleteTestSuiteElement(final IPackageFragment packageOfDeletedClass,
		...
		JavaCore.run(new IWorkspaceRunnable() {
			...
			public void run(IProgressMonitor monitor) throws CoreException {
				...
				String testElementList = createCommaSeparatedClassList(packageOfDeletedClass.getCompilationUnits(),
						null, deletedClass.getElementName().replace(".java", ""));

				IType testSuiteType = refreshTestSuiteElements(testSuite, TESTSUITE_PREFIX, testElementList);

				// if only the test-class and the test-suite is in the package,
				// then delete the complete package
				if (testSuiteType == null && packageOfDeletedClass.getCompilationUnits().length == 2) {
	...
	protected ICompilationUnit getTestSuite(IPackageFragment p) throws JavaModelException {
		return p.getCompilationUnit(TESTSUITE_PREFIX + ".java");
	}

	private IType refreshTestSuiteElements(ICompilationUnit testSuite, String testSuiteName, String testElementList)
			throws JavaModelException {
		...
		if (testElementList.length() > 0) {
			...
			testSuiteType = testSuite.createType(
